...
async function getBalances(consentId, token) {
  let response;
  try {
    response = await callBalances(consentId, token);
    return response;
  } catch (error) {
    return error;
  }
...
async function callBalances(consentId, token) {
  let body = '';
  ...
  console.log(`Calling CONSENT_API ${ENV_API}/v1/340477c5-f088-4d5d-9dcc-e7adf27547b5/Balances/`);
  let result;
  try {
    result = await utils.manageRequestResponse(
      new URL(`${ENV_API}/v1/Accounts/340477c5-f088-4d5d-9dcc-e7adf27547b5/Balances/`),
      {
